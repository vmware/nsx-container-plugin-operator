...
func TestBuildWorkaroundPolicyFiltersIPv6(t *testing.T) {
	netpol := buildWorkaroundPolicy(testCanaryNetPol(), []string{"192.168.16.22", "2001:db8::1", "10.0.0.1"}, []int32{8443})
	...
	assert.ElementsMatch(t, []string{"192.168.16.22/32", "10.0.0.1/32"}, cidrs)
}
...
func TestCanaryServicePorts(t *testing.T) {
	...
	t.Run("prefers TargetPort over Port", func(t *testing.T) {
		svc := canaryService(
			corev1.ServicePort{Port: 8443, TargetPort: intstr.FromInt32(8080), Protocol: corev1.ProtocolTCP},
			corev1.ServicePort{Port: 9000, Protocol: corev1.ProtocolTCP},
		)
		r := fakeReconciler(svc)
		ports, err := r.canaryServicePorts(context.TODO())
		require.NoError(t, err)
		assert.ElementsMatch(t, []int32{8080, 9000}, ports)
	})
